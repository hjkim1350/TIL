...
```sql
SELECT * FROM healthcare WHERE waist != "" AND is_drinking = 1 ORDER BY waist DESC LIMIT 5;
```
...
### 13. (자유문제) 흡연자(smoking 1) 중 몸무게가 가장 많은 5명의 키를 출력

```sql
SELECT height FROM healthcare WHERE smoking = 1 ORDER BY weight DESC LIMIT 5;
```
